...
def get_rooms(connection):
    with connection.cursor() as cursor:
        sql = 'SELECT `id`, `name`, `ip`, `boiler`, `humidifier` FROM `rooms`;'
        cursor.execute(sql)
        rs = cursor.fetchall()
...
def get_room(id, connection):
    with connection.cursor() as cursor:
        sql = '''
            ...
            `max_humidity`, `min_humidity`, `boiler`, `humidifier`
...
def get_room_ip(id, connection):
    with connection.cursor() as cursor:
        sql = '''
            SELECT `id`, `ip` FROM `raspi`.`rooms` WHERE id = %s;
        '''
        cursor.execute(sql, id)
        row = cursor.fetchone()
